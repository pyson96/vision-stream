...
class BasePredictor:
    ...
    def setup_source(self, source):
        ...
        self.vid_writer = {}
    def setup_ffmpeg_process(self, width, height, fps):
        ffmpeg_cmd = [
            'ffmpeg', '-y',
            '-f', 'rawvideo',
            '-pix_fmt', 'bgr24',
        ...
        print("ffmpeg command registered")
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            # Setup stream writer once per inferences
            height, width = self.dataset.im0[0].shape[:2]
            fps = self.args.fps or 25.0
            ffmpeg_process = self.setup_ffmpeg_process(width, height, fps)
            for self.batch in self.dataset:
                ...
                for i in range(n):
                    ...
                    if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                        ...
                        ffmpeg_process.stdin.write(self.plotted_img.tobytes())
                # Print batch results
                if self.args.verbose:
                    LOGGER.info("\n".join(s))
